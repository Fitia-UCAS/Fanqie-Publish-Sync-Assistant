...
from backend.syncing.local_source import Chapter, parse_chapters
from backend.errors import ErrorStage
from backend.syncing.plan import SyncPlan, SyncedChapter
from backend.syncing.plan import make_sync_plan
from backend.syncing.chapter import run_single_chapter_sync
...
from backend.fanqie_web.ui_actions import dismiss_popups, goto_chapter_manage
from backend.fanqie_web.chapter_list import build_chapter_editor_index
from backend.fanqie_web.schedule import build_schedule_slots, describe_schedule_slots
from backend.novel.source import chapters_by_number
from backend.paths import SYNCING_COMPARE_DIR, SYNCING_DEBUG_DIR, SYNCING_HISTORY_DIR
...
def run_multi_chapter_sync(
    ...
    if options.debug_screenshots:
        log(f"番茄同步调试截图已开启：{SYNCING_DEBUG_DIR}")
    else:
        log("番茄同步调试截图已关闭。")
    if options.git_tracking:
        log(f"番茄同步 Git追踪已开启：{SYNCING_HISTORY_DIR}")
    else:
        log("番茄同步 Git追踪已关闭。")
...
class MultiChapterSyncState:
    def __init__(self, chapters: list[int], results: list[SyncedChapter], result_chapter_numbers: list[int]) -> None:
        self.chapters = chapters
        self.results = results
        self.result_chapter_numbers = result_chapter_numbers

    def append(self, chapter_no: int, result: SyncedChapter) -> None:
...
def _clean_previous_sync_artifacts(*, log: Callable[[str], None]) -> None:
    ...
    for directory, label in (
        (SYNCING_DEBUG_DIR, "调试截图"),
        (SYNCING_COMPARE_DIR, "对比报告"),
        (SYNCING_HISTORY_DIR, "Git追踪"),
    ):
        try:
    ...
    *,
    ...
    for index, chapter_no in enumerate(chapters, start=1):
        if is_stop_requested(stop_requested):
            log("已停止同步。")
            break
        wait_while_paused(pause_requested=pause_requested, stop_requested=stop_requested, log=log, label="同步")
        if is_stop_requested(stop_requested):
            log("已停止同步。")
            break
...
def _record_chapter_failure(*, page, chapter_manage_url: str, chapter_no: int, exc: Exception, state: MultiChapterSyncState, log: Callable[[str], None]) -> None:
    ...
    log(msg)
    state.append(chapter_no, SyncedChapter(ok=False, changed=False, published=False, message=msg, error_stage=ErrorStage.CHAPTER))
    try:
